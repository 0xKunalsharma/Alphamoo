...
class AgentStateTracker:
    ...
    Two modes of operation:
      1. MOVEMENT MODE (default): for games with directional actions (1-4).
         Uses movement-correlation to find the agent.
      2. CLICK CURSOR MODE: for click-only games (available_actions has 6
         but no 1-4). The "agent" is the cursor — we track last click position.
         No persistent agent object exists; color/shape/orientation are N/A.
    ...
    def __init__(self, background_color: int | None = None):
        ...
        self.click_cursor_mode: bool = False  # click-only game; track cursor
        self.click_game_mode: bool = False  # legacy flag (set when action_id==6)
    ...
    def update(self, grid: list[list[int]] | np.ndarray,
        ...
        Args:
            ...
            available_actions: list of valid action IDs for this game.
                Used to detect click-only games on first call.
            action_input: the full ActionInput object (for extracting click coords).
        ...
        # Detect click-only games on first call
        if self.prev_grid is None and available_actions is not None:
            has_movement = any(a in ACTION_DISPLACEMENTS for a in available_actions)
            has_click = 6 in available_actions
            if has_click and not has_movement:
                self.click_cursor_mode = True

        # --- CLICK CURSOR MODE ---
        # For click-only games, the agent IS the cursor. Track last click position.
        if self.click_cursor_mode:
            return self._update_click_cursor(grid, action_id, action_input, diag)

        # --- MOVEMENT MODE (original logic) ---
        ...
        # Fallback: hybrid games (movement + click) where this action was a click.
        # Propagate previous agent state; check for color/shape changes at same location.
        if new_state is None and self.agent_state is not None:
            self.click_game_mode = (action_id == 6)
            propagated_cells = self._propagate_from_previous_state(grid)
            if propagated_cells:
    ...
    def _update_click_cursor(self, grid: np.ndarray, action_id: int,
                              action_input: object | None,
                              diag: TrackerDiagnostics
                              ) -> tuple[AgentState | None, TrackerDiagnostics]:
        """Handle click-only games: track cursor position as agent state."""
        # Extract click coords if this was a click action
        click_coords = None
        if action_input is not None:
            click_coords = getattr(action_input, "click_coords", None)
        elif action_id == 6:
            # Fallback: can't get coords without action_input
            pass

        if click_coords is not None:
            self.last_click_position = click_coords
            self.click_history.append(click_coords)

        # Build agent state from cursor position
        if self.last_click_position is not None:
            self.agent_state = AgentState(
                object_ids=[],  # no persistent agent object in click games
                position=self.last_click_position,
                orientation=0,  # N/A for cursor
                shape="",       # N/A for cursor
                color=[],       # N/A for cursor
                energy=None,
                inventory=[],
            )
            diag.detected = True
            diag.detection_method = "click_cursor"
            diag.n_agent_cells = 0  # cursor has no cells
            diag.agent_colors = []
            diag.bbox = (self.last_click_position[0], self.last_click_position[1],
                        self.last_click_position[0], self.last_click_position[1])
            self._detection_count += 1
            self._click_cursor_count += 1
        else:
            # No click yet (first frame or non-click action) — no agent state
            if self.agent_state is None:
                diag.notes = "click game: no click yet"
    ...
    def _propagate_from_previous_state(self, grid: np.ndarray) -> set[tuple[int, int]] | None:
        """Find cells in current grid that match the previous agent's location/colors."""
        if self.agent_state is None or not self.agent_state.color:
            return None
    ...
    def get_stats(self) -> dict:
        ...
        return {
            ...
            "click_cursor_mode": self.click_cursor_mode,
            "click_cursor_count": self._click_cursor_count,
        }
