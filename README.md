...
## Performance Benchmarks (Phase 0 preview)

| Operation | Avg time | Budget % (1.85s/action) |
|-----------|----------|--------------------------|
| Vortex load (per file) | ~50ms | 2.7% |
| Perception (per frame) | 3.71ms | 0.2% |
| Cascade interpretation (per cascade) | ~5-15ms | <1% |
| Frame rendering (PNG) | ~2ms | 0.1% |

**Headroom remaining for LLM reasoning: ~95% of action budget.** Confirms our Phase 0 hypothesis — at 0.5B-1.5B model size, we can comfortably fit the 9-hour Kaggle budget.
